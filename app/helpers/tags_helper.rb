module TagsHelper
  def taggable_link model
    case model
    when Article, Post
      link_to h(model.title), (send "#{model.class.to_s.downcase}_url", model)
    when Topic
      ...
      link_to h(model.title), forum_topic_url(model.forum, model)
    when Issue
      link_to h(model.summary), issue_url(model)
    else
