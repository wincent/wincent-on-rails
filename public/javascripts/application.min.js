...
$.fn.relativizeDates=function(){$(".relative-date").removeClass("relative-date").addClass("relativized-date").each(function(){var a=this.innerHTML,b=new Date,c;c=(c=a.match(/(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2}) UTC/))?new Date(Date.UTC(parseInt(c[1]),parseInt(c[2],10)-1,parseInt(c[3],10),parseInt(c[4],10),parseInt(c[5],10),parseInt(c[6],10))):new Date;b=b.getTime()-c.getTime();b=Math.round(b/1E3);b<0?a="in the future":b===0?a="now":b<60?a="a few seconds ago":b<120?a="a minute ago":b<180?
a="a couple of minutes ago":b<300?a="a few minutes ago":b<3600?a=Math.round(b/60)+" minutes ago":b<7200?a="an hour ago":b<86400?a=Math.round(b/3600)+" hours ago":(b=Math.round(b/86400),b===1?a="yesterday":b<=7?a=b+" days ago":(b=Math.round(b/7),a=b===1?"a week ago":b<=6?b+" weeks ago":c.toLocaleDateString()));this.title=c.toLocaleString();this.innerHTML=a})};
