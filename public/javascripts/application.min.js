...
keyword:/\b(_Bool|bool|char|const|int|long|short|struct|typedef|unsigned|void)\b/,statement:/\b(break|case|continue|default|do|else|for|goto|if|return|sizeof|static|switch|while)\b/,"boolean":/\b(true|false)\b/,"numeric-literal":/\b(0[xX][a-fA-F0-9]+|\d+(\.\d+f?)?)\b/};b["diff-syntax"]={"skip-header":/^(---|\+\+\+) .*/m,deleted:/^-.*/m,added:/^\+.*/m,"hunk-header":/^@@ -\d+(,\d+) \+\d+(,\d+) @@.*/m};b["objc-syntax"]=$.extend({},b["c-syntax"],{keyword:/\b(_Bool|BOOL|bool|char|const|int|long|short|struct|typedef|unsigned|void)\b/,
"boolean":/\b(YES|NO|true|false)\b/});b["ruby-syntax"]={"skip-anchor":/<a .+?a>/,"skip-entities":/&\w+;/,comment:/#.*/,"string-literal":/("(<a .+?a>|[^"\\]|\\.)*"|'(<a .+?a>|[^'\\]|\\.)*')/,keyword:/\b(begin|break|catch|class|continue|def|else|end|for|if|include|load|module|raise|redo|require|rescue|retry|then|throw|unless|until|while|yield)\b/,statement:/\b(private|protected|public)\b/,"boolean":/\b(true|false)\b/,constant:/\b[A-Z][a-zA-Z0-9_]*\b/,namespace:/::/,identifier:/(\$|@{1,2})[a-z_][a-zA-Z0-9_]*\b/,
symbol:/:[a-zA-Z_][a-zA-Z0-9_]*\b/,"numeric-literal":/\b(0[xX][a-fA-F0-9]+(_[a-fA-F0-9]+)*|0[bB][0-1]+(_[0-1]+)*|[0-9]+(_[0-9]+)*(\.[0-9]+(_[0-9]+)*)?)\b/};b["shell-syntax"]={output:/^[^#\$\n].*/m,constant:/(\\\n|.)+/m};this.find("pre[class$=syntax][class!=highlighted]").each(function(){var c=b[$(this).attr("class")];this.innerHTML='<span class="line-number"></span>'+a(this.innerHTML,c).split("\n").join('\n<span class="line-number"></span>');$(this).addClass("highlighted")});return this};
function setUpSearchLink(){$("#search-link").click(function(){$("#search_box").toggle().focus();return false})}function highlightActiveFragment(){var a=window.location.hash;a.length>=2&&$(a).addClass("active-fragment")}
$(document).ready(function(){setUpLoginLogoutLinks();setUpSearchLink();displayCacheableFlash();relativizeDates();highlightActiveFragment();$("body").syntaxHighlight();$("a[data-confirm],input[data-confirm]").live("click",function(){return confirm($(this).attr("data-confirm"))});$("a[data-method=delete]").live("click",function(a){var b=$("meta[name=csrf-token]").attr("content"),c=$("meta[name=csrf-param]").attr("content"),e=$(this),d=e.attr("href");e=e.attr("data-method");d=$("<form/>",{method:"post",
action:d,type:"hidden"}).append($("<input/>",{name:"_method",value:e,type:"hidden"}));b&&c&&d.append($("<input/>",{name:c,value:b,type:"hidden"}));d.hide().appendTo("body");d.submit();a.preventDefault()});$("a[data-popup]").live("click",function(){window.open(this.href,null,$(this).attr("data-popup"));return false})});
