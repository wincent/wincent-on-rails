...
function syntaxHighlight(a,b){if(!b)return a;for(var c="";a!="";){var d=null,e=null;for(var g in b){var f=a.match(b[g]);if(f&&(d&&f.index<d||!d)){d=f.index;f.rule=g;e=f;if(d==0)break}}if(e){d='<span class="'+e.rule+'-syntax">';f="</span>";if(e.rule.match(/^skip-/))f=d="";c=c+a.substring(0,e.index)+d+e[0]+f;a=a.substring(e.index+e[0].length)}else{c+=a;break}}return c}
...
symbol:/:[a-zA-Z_][a-zA-Z0-9_]*\b/,"numeric-literal":/\b(0[xX][a-fA-F0-9]+(_[a-fA-F0-9]+)*|0[bB][0-1]+(_[0-1]+)*|[0-9]+(_[0-9]+)*(\.[0-9]+(_[0-9]+)*)?)\b/};a["shell-syntax"]={output:/^[^#\$\n].*/m,constant:/(\\\n|.)+/m};$("pre[class$='syntax']").each(function(){this.innerHTML='<span class="line-number"></span>'+syntaxHighlight(this.innerHTML,a[$(this).attr("class")]).split("\n").join('\n<span class="line-number"></span>')})}
function setUpSearchLink(){$("#search-link").click(function(){$("#search_box").toggle().focus();return false})}$(document).ready(function(){setUpLoginLogoutLinks();setUpSearchLink();displayCacheableFlash();relativizeDates();stylePreBlocks();$("a[data-confirm],input[data-confirm]").live("click",function(){return confirm($(this).attr("data-confirm"))})});
