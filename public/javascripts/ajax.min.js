...
function ajax_comment_form(a){var b=$("#comment-form"),d=b.find("a");d.attr("href","#comment-form");var f=function(){d.unbind("click").addClass("disabled");var h=$("<img/>",{alt:"Spinner",id:"spinner_"+global_spinner_counter++,"class":"spinner",src:"/images/spinner.gif"});b.append(h);h.show();clearAJAXFlash();$.ajax({url:a,type:"get",dataType:"html",success:function(a){clearAJAXFlash();b.find(".links").hide();b.append(a)},error:function(a){insertAJAXFlash("error",a.responseText);d.click(f)},complete:function(){h.remove();
d.removeClass("disabled")}})};d.click(f)}
function edit_in_place(a,b,d,f){$(a).each(function(){function a(){e.addClass("highlight")}function g(){e.removeClass("highlight")}function i(){var c=e.text();e.unbind("mouseenter mouseleave");g();e.attr("title","Click outside to abort editing");e.html('<form action="javascript:void(0)" style="display:inline;"><input type="text" value="'+escapeHTML(c)+'"></form>');e.find("input")[0].select();e.unbind("dblclick");e.find("input").blur(function(){e.text(c);e.dblclick(i);e.hover(a,g)});e.find("form").submit(function(){var c=
e.find("input").val();e.text("saving...");$.ajax({url:f+j,type:"post",dataType:"json",data:"_method=put&"+b+"["+d+"]="+encodeURIComponent(c),success:function(a){e.text(a[b][d]);e.removeClass("ajax_error");clearAJAXFlash()},error:function(a){e.text(c);e.addClass("ajax_error");insertAJAXFlash("error",a.responseText)},complete:function(){e.hover(a,g);e.dblclick(i)}})})}var c=$(this).attr("id"),j=c.match(/_(\d+)$/)[1],e=$("#"+c+"_"+d);e.attr("title","Double-click to edit");e.dblclick(i);e.hover(a,g)})}
function ajax_check_box(a,b,d,f){a=$(a).attr("id");var h=a.match(/_(\d+)$/)[1],g=$("#"+a+"_"+d);a=g.text();var i="spinner_"+global_spinner_counter++;g.html('<input id="'+b+"_"+d+'" name="'+b+"["+d+']" type="checkbox"><img alt="Spinner" id="'+i+'" class="spinner" src="/images/spinner.gif" />');var c=g.find("input");a==="true"&&c.attr("checked","checked");var j=c.attr("checked");c.change(function(){var a=g.find("#"+i);a.show();$.ajax({url:f+h,type:"post",dataType:"json",data:"_method=put&"+b+"["+d+
