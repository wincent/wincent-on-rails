...
function ajax_comment_form(a){var b=$("#comment-form"),e=b.find("a");e.attr("href","#comment-form");var f=function(){e.unbind("click").addClass("disabled");var h=$("<img/>",{alt:"Spinner",id:"spinner_"+global_spinner_counter++,"class":"spinner",src:"/images/spinner.gif"});b.append(h);h.show();clearAJAXFlash();$.ajax({url:a,type:"get",dataType:"html",success:function(a){clearAJAXFlash();b.hide().after(a)},error:function(a){insertAJAXFlash("error",a.responseText);h.remove();e.click(f).removeClass("disabled")}})};
e.click(f)}
