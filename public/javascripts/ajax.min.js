...
function ajax_comment_form(a){var c=$("#comment-form"),d=c.find("a");d.attr("href","#comment-form");var e=function(){d.unbind("click");d.addClass("disabled");var j="spinner_"+global_spinner_counter++;c.append('<img alt="Spinner" id="'+j+'" class="spinner" src="/images/spinner.gif" />');var g=c.find("#"+j);g.show();clearAJAXFlash();$.ajax({url:a,type:"get",dataType:"html",success:function(h){clearAJAXFlash();c.html(h)},error:function(h){insertAJAXFlash("error",h.responseText);g.remove();d.click(e);
d.removeClass("disabled")}})};d.click(e)}
