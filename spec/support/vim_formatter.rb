    ...
    module Formatters
      class VimFormatter < BaseTextFormatter
        ...
        def close
          ...
          if failure_count > 0
            system "growlnotify -n autotest --image ./autotest/fail.png -p Emergency -m '#{summary}' -t 'Spec failure detected'"
          elsif pending_count > 0
            system "growlnotify -n autotest --image ./autotest/pending.png -p High -m '#{summary}' -t 'Pending spec(s) present'"
          else
            system "growlnotify -n autotest --image ./autotest/pass.png -p 'Very Low' -m '#{summary}' -t 'All specs passed'"
          end
        end
